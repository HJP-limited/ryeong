...
| 상태 | **스크립트가 아니라 직접 Edit으로 이뤄져 이 폴더에 복원되지 않았다.** 세션 기록 8/25~8/26 구간에서 추가 추출이 필요하다 |

관련 파일: `chain.py`, `chain_search.py`, `chain_seed.py`(미복원), `kt_tests.py`
...
3. 동명이인(2번), 주제 전환(4번) — 표본이 작아(동명이인 채점 n=6) 효과 측정이 어렵다. 여유 있을 때
